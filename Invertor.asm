...
_start:
	...
	label_1:
		mov rbx, rcx
		dec rbx ; rbx = rcx - 1
		mov al, [var_read + rbx] ; символ, обрабатываемый в данной итерации цикла, находится в однобайтном регистре al
		...
		jmp not_a_letter ; если предыдущие проверки не прошли, то мы имеем не что иное, как один из сиволов с кодом в диапазоне от 91 до 96: в таком случае, это тоже не латинская буква

	not_a_letter:
		mov [var_write + rbx], al ; если обрабатываемый символ -- не латинская буква, то без изменений копируем его в соответствующий байт переменной var_write
		cmp rcx, 2
	...
	from_low_to_upper:
		sub al, 'a' - 'A'
                ...
                loop label_1 ; здесь предпроверку можно не делать: последующих меток нет, так что при занулении rcx команда loop не сработает, зато сразу осуществится переход к выводу var_write на экран
	end:	
